...
| Twilio                                   | ^6.1.0                    | Phone verification (Verify API) for posting a `ServiceRequest` — no-ops without `TWILIO_*` env vars        | `src/lib/sms.ts`                                                     |
...
45 models across `prisma/schema.prisma` (1444 lines). Full ER diagram below;
...
erDiagram
    ...
    Profile ||--o{ Album : "has"
    Album ||--o{ ProfileMedia : "groups"
    Profile ||--o{ Service : "offers"
    User ||--o{ ServiceRequest : "posts (as customer)"
    ServiceRequest ||--o{ RequestOffer : "receives"
    User ||--o{ RequestOffer : "sends (as provider)"
    ServiceRequest ||--o| Booking : "fulfilled by"
...
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `User`                                                 | `email` (unique), `passwordHash?`, `username?` (unique), `deletedAt?` (soft delete), `acceptingBookings`, `notificationPreferences` (Json), `isSuspended`/`isVerified` (admin moderation), `phoneVerified` (Twilio Verify — gates posting a `ServiceRequest`, see `lib/sms.ts`) | One row per person, regardless of role count                                                                                                                                                                                        | —                                                                                                              |
...
| `ProfileMedia`                                         | `profileId`, `albumId?`, `url`, `publicId` (Cloudinary), `type`, `order`, `deletedAt?` (7-day recoverable trash, see `services/albums.ts`)                                                                                                                                      | Portfolio images/videos                                                                                                                                                                                                             | `[profileId, order]`, `[albumId]`, `[deletedAt]`                                                               |
| `Album`                                                | `profileId`, `title`, `category?` (nullable only for the one migration-created "Ảnh chưa phân loại" legacy album — new albums always require it), `coverMediaId?`, `sortOrder`, `deletedAt?`                                                                                    | Groups `ProfileMedia` into a titled, browsable portfolio unit                                                                                                                                                                       | `[profileId, sortOrder]`, `[deletedAt]`                                                                        |
...
| `ServiceRequest`                                       | `code` (human-readable, e.g. `YC-2026-00042`), `customerId`, `role` (single — simplified from the source prompt's multi-role design), `categories`, `detailedAddress?` (withheld until an offer is accepted), `status`, `expiresAt` (7 days)                                    | Reverse marketplace (Prompt G7) — customer posts a need, matching providers offer                                                                                                                                                   | `[customerId, status]`, `[role, provinceId, status]`, `[status, expiresAt]`                                    |
| `RequestOffer`                                         | `requestId`, `providerId`, `proposedPrice`, `status`                                                                                                                                                                                                                            | One provider's offer against a `ServiceRequest` — accepting one creates a `Booking` via the existing `createBooking()`, never a parallel flow                                                                                       | `@@unique([requestId, providerId])`, `[providerId, status]`                                                    |

...
export function AvailabilitySettings() {
      ...
      .then((body) => {
        ...
        startTransition(() => {
          if (body.data?.schedule?.length > 0) {
            const byDay = new Map(body.data.schedule.map((s: DaySchedule) => [s.dayOfWeek, s]));
            setSchedule(
              DEFAULT_SCHEDULE.map((d) => {
                const existing = byDay.get(d.dayOfWeek) as DaySchedule | undefined;
                return existing
          ...
          setBlockedDates(
            (body.data?.blockedDates ?? []).map((b: { id: string; date: string; reason: string | null }) => ({
              id: b.id,
  ...
  const addBlockedDate = async () => {
    ...
    if (res.ok) {
      setBlockedDates((prev) => [...prev, { ...body.data, date: newBlockedDate }]);
      setNewBlockedDate("");
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        ...
        {schedule.map((day) => (
          <div key={day.dayOfWeek} className="flex items-center gap-3">
            <Switch
              ...
              onChange={(value) => updateDay(day.dayOfWeek, { isActive: value })}
            />
        ...
        <Button variant="secondary" size="sm" className="self-start" disabled={isSaving} onClick={save}>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          {blockedDates.map((d) => (
            ...
            >
              {d.date}
              <button type="button" onClick={() => removeBlockedDate(d.id)} aria-label="Remove">

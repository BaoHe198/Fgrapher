...
export function NotificationsSettings({
  ...
  return (
    // One table, one header row. Each group used to be its own grid with its
    // own "Email / Trong ứng dụng" header — the same two column titles five
    // times down the page.
    <div className="grid grid-cols-[1fr_auto_auto] items-center gap-x-6 gap-y-3">
      <span />
      <span className="text-body-sm text-text-tertiary">{t("email")}</span>
      <span className="text-body-sm text-text-tertiary">{t("inApp")}</span>

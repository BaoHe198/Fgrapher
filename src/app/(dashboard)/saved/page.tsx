...
export default async function SavedProfilesPage() {
  ...
  const profiles = await db.profile.findMany({
    ...
    include: { user: { select: { username: true, name: true, firstName: true, lastName: true } } },
  });
    ...
    <div className="flex flex-col gap-5">
      ...
      {profiles.length === 0 ? (
        <div className="flex flex-col items-center gap-3 py-16 text-center">
          ...
          <p className="text-body-lg font-semibold text-text-primary">No saved artists yet</p>
          ...
          {profiles.map((profile) => (
            <ArtistCard
              ...
              artist={{
                ...
                rating: "—",
                reviews: 0,
                price: profile.priceMin ? `From ₫${profile.priceMin.toLocaleString()}` : "",
              }}
            />
          ))}
        </div>

    ...
    <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-8 sm:py-8">
      <div className="relative mb-6 overflow-hidden rounded-[var(--fg-radius-xl)] border border-border-subtle bg-brand-primary px-5 py-6 text-text-on-brand sm:px-8 sm:py-8">
        ...
        <div className="relative max-w-2xl">
          <p className="mb-2 text-body-sm font-semibold! tracking-wide text-gold-300 uppercase dark:text-text-on-brand/80">
            {t("eyebrow")}
          </p>
          <h1 className="text-display-md">{t("heading")}</h1>
          {/* text-on-brand, not white: brand-primary turns light teal in dark
              mode and its on-brand text turns dark with it. */}
          <p className="mt-2 text-body-md text-text-on-brand/75">
            {t("subtitle")}
          </p>
        </div>
      </div>

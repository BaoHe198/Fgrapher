...
export async function getAdminStats() {
  ...
  const [
    ...
    db.user.count({ where: { deletedAt: null, createdAt: { lt: monthStart, gte: lastMonthStart } } }),
    db.subscription.findMany({
      where: { status: { in: ["ACTIVE", "TRIALING"] } },
      include: { userRole: { select: { role: true } } },
    }),
    db.booking.count({ where: { createdAt: { gte: monthStart } } }),
    db.booking.count({ where: { createdAt: { gte: monthStart }, status: "COMPLETED" } }),
  ...
  return {
    totalUsers,
    userGrowthPercent: usersLastMonth > 0 ? Math.round(((totalUsers - usersLastMonth) / usersLastMonth) * 100) : 0,
    activeSubscriptions: activeSubscriptions.length,
    mrr,
    bookingsThisMonth,
    completionRate:
      bookingsThisMonth > 0 ? Math.round((completedBookingsThisMonth / bookingsThisMonth) * 100) : 0,

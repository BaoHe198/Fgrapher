...
export function ProductCard({ product }: ProductCardProps) {
  const shopName = product.user.firstName ?? product.user.name ?? "Shop";
    ...
    <Link href={`/shop/${product.id}`}>
      ...
      >
        <div className="relative aspect-[4/3] w-full">
          ...
          <span className="absolute top-2.5 left-2.5">
            <Badge variant={product.type === "RENT" ? "accent" : "neutral"}>
              {product.type === "RENT" ? "Rental" : product.type === "BOTH" ? "Sale & Rental" : "For sale"}
            </Badge>
          </span>
          {outOfStock ? (
        ...
        <div className="flex flex-col gap-1.5 p-3.5">
          <span className="line-clamp-2 text-heading-sm text-text-primary">{product.name}</span>
          <Badge variant="neutral" className="w-fit">
            {CONDITION_LABEL[product.condition] ?? product.condition}
          </Badge>
          <span className="text-body-md font-semibold text-text-primary">
            {product.type === "RENT" && product.rentalPrice
              ? `${formatCurrency(product.rentalPrice, product.currency)}/day`
              : product.price

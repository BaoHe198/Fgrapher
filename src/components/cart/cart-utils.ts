...
export function groupByShop(items: CartItemWithProduct[]) {
  const groups = new Map<string, { shopName: string; items: CartItemWithProduct[] }>();
  for (const item of items) {
    const shopId = item.product.user.id;
    const shopName =
      item.product.user.profiles[0]?.shopName ?? item.product.user.firstName ?? item.product.user.name ?? "Shop";
  ...
  if (item.type === "RENT") {
    const days =
      item.rentalStart && item.rentalEnd
        ? Math.max(1, Math.round((new Date(item.rentalEnd).getTime() - new Date(item.rentalStart).getTime()) / 86_400_000))
        : 1;
...
export function cartTotals(items: CartItemWithProduct[]) {
  ...
  const deposits = items
    ...
    .reduce((sum, item) => sum + (item.product.depositAmount ?? 0) * item.quantity, 0);
